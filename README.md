...
### 🗂 일괄 PDF (폴더로 여러 문서 추가 후)
- **전체 합쳐 1개 PDF**: 등록된 모든 .md를 문서마다 새 페이지로 이어 붙여 하나의 PDF로 인쇄(1회). "파일명 제목 머리말"을 켜면 각 문서 앞에 파일명 제목이 붙습니다.
- **문서별 개별 PDF**: 문서마다 저장 대화상자가 한 번씩 뜹니다(파일명은 문서명으로 자동 제안). 브라우저 특성상 파일별 저장은 수동 확인이 필요합니다.
