...
- 변환 파이프라인 **순서 고정**: markdown-it(texmath) → DOMPurify → DOM 주입 → 이미지 src 치환 → highlight.js → Mermaid → `document.fonts.ready` → Paged.js.
...
import {
  ...
  buildRegistry, resolveImagePath,
} from '../src/files.js'
...
- Produces:
  - `createRenderer(deps): { renderMarkdown(text: string): string }`
    - `deps = { markdownit, texmath, katex, hljs, DOMPurify }`
    - `renderMarkdown`은 markdown-it(texmath+highlight) 렌더 후 DOMPurify로 정화한 **HTML 문자열** 반환.
...
const renderer = createRenderer({ markdownit, texmath, katex, hljs, DOMPurify })
...
export const createRenderer = (deps) => {
  const { markdownit, texmath, katex, hljs, DOMPurify } = deps
...
```js
import { buildRegistry, dirname } from './src/files.js'
...
// --- 파일 등록 ---
const addEntries = (fileList) => {
  const entries = Array.from(fileList).map(f => ({
    path: f.webkitRelativePath || f.name,
    file: f,
  }))
  const reg = buildRegistry(entries)
  // 누적: 기존 + 신규(키 중복 시 신규로 교체)
...
const renderFileList = () => {
  ...
  for (const m of state.registry.mdFiles) {
    const item = document.createElement('div')
    item.className = 'file-item' + (m.key === state.selectedKey ? ' selected' : '')
    item.textContent = m.key
    item.onclick = () => selectFile(m.key)
...
```css
#file-list .file-item { padding: 6px 8px; border-radius: 6px; cursor: pointer; font-size: 13px; word-break: break-all; }
...
## 등록
- [ ] 📁 폴더 선택으로 `fixtures/sample` 폴더 선택 → 목록에 `sample/note.md` 표시
...
```bash
git add fixtures docs/MANUAL-TEST.md
git commit -m "test: 종단 수동 검증 픽스처 + 체크리스트"
```
